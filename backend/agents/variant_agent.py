...
_NON_EU_OFFICES = {"GB", "CH", "TR", "UA", "MD", "US", "SA", "RU", "AM", "VN", "CN"}
...
def build_offices_and_territories(user_offices: List[str]):
    ...
    Logică:
    - EU_FULL  → 24 state UE + BX (BE/NL/LU → BX, singurul cod valid TMview) + EM + offices=["WO"] = 26 teritorii
    ...
    - Oficiu non-UE (GB/CH/TR/UA/MD/US/SA/RU/AM/VN/CN) → offices = [cod]
    ...
    for code in user_offices:
        c = code.upper()
        if c == "EU_FULL":
            # 24 state UE individuale + BX (BE/NL/LU → BX, codul valid TMview pt Benelux)
            # + EM (EUIPO) = 26 teritorii TMview valide → 4 batch-uri de câte 7
            # BE, NL, LU NU sunt coduri valide în TMview — batch-urile cu ele returnează 400
            for eu_c in ALL_EU_TERRITORIES:
                if eu_c in _BENELUX:
                    territories_set.add("BX")
                else:
                    territories_set.add(eu_c)
            ...
            # territories=["EM"] e valid în TMview — returnează mărci EUIPO și WIPO/EU.
        ...
        elif c in _NON_EU_OFFICES:
            # Oficii non-UE (GB/CH/TR/UA/MD/US/SA/RU/AM/VN/CN) — TMview le are ca
            # participating offices, dar NU ca teritorii UE. territories=[cod] pentru
            # aceste coduri nu era valid și întorcea mereu 0 rezultate (bug: acest
            # branch exista definit mai sus dar nu era niciodată folosit — orice
            # căutare pe aceste țări pica silențios pe date demo).
            offices_set.add(c)
        else:
            territories_set.add(c)
